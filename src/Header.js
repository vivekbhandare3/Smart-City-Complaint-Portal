...
const Header = ({ user, userData, onLogout, onNormalLoginClick, onGovLoginClick }) => (
  <header className="bg-white shadow-md">
    <div className="container mx-auto px-0 py-4 flex justify-between items-center">
      <div className="text-2xl font-bold">
        <span className="text-blue-700">Smart City</span> Complaint Portal
      </div>
      {user ? (
          ...
          <span className="mr-4 hidden md:block">
            ...
            {userData?.role === 'admin' && (
              <span className="ml-2 bg-green-500 text-white px-2 py-1 rounded-full text-xs font-bold">
                Government Admin{userData?.department ? ` (${userData.department})` : ''}
              </span>
            )}
          </span>
          <button
